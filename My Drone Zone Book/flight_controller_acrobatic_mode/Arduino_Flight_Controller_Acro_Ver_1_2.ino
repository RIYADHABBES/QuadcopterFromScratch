// ################################################################
// ####                                                        ####
// ####       Program "Flight Controller" for Arduino V1.2     ####
// ####                                                        ####
// ####             PID control of angular velocities          ####
// ####                                                        ####
// ####                Piloting in acrobatic mode              ####
// ####                                                        ####
// ################################################################
...
const float KpRoll = 1.30 ;  // value of proportional gain in roll
const float KiRoll = 0.04;   // value of integral gain in roll
const float KdRoll = 18.0;   // value of derivative gain in roll
...
const float KpYaw = 4.0;   // value of proportional gain in yaw
const float KiYaw = 0.02;  // value of integral gain in yaw
const float KdYaw = 0.0;   // value of derivative gain in yaw
...
//------------------------------------------------
// MPU6050 reading and data processing variables 
//------------------------------------------------

int gyro_raw[3] = {0,0,0};  // table of raw angular velocities

float gyro_offset[3] = {0,0,0}; // table of gyroscope offsets
...
void getAngularVelocities();  // processing and filtering of raw gyroscope data to obtain reliable angular rate values
...
void loop() 
{
  ...
  // processing of gyroscope measurements to obtain reliable angular velocity values    
  ...
  // the duration of the "loop()" loop varies between 1.29ms and 2.75ms (duration measured at the oscillocsope)
  // according to modes ("STOP", "ARMED", "RUN)
  // its duration is therefore calibrated at 3000us so that it is fixed in order to obtain optimal PID regulation

  // we wait as long as the execution time of the "loop()" has not reached 3000us
  while(micros()-loop_start < 3000);
...
void readMPU6050()
{
    ...
    Wire.beginTransmission(MPU_ADDRESS);
    Wire.write(0x43); // start address for angular velocity values
    Wire.endTransmission();             
    Wire.requestFrom(MPU_ADDRESS,6); // 6 byte reading request
  
    // waiting until all 6 bytes are received
    ...
    gyro_raw[YAW] = Wire.read() << 8 | Wire.read();  // gyro yaw
	  ...
	  // changes of signs for our rotation direction convention
...
void calibrateMPU6050()
{
    ...
    {
        // the LED flashes quickly during calibration
        // to do this, we make it change state every 25 iterations of the "for" loop.
        if(i % 25 == 0) digitalWrite(LED, !digitalRead(LED));   
    ...
    // calculation of the average offsets for the gyroscope
...
//################################################################
//  Gyroscope measurement processing function to obtain reliable
...
void getAngularVelocities()
{
    ...
    // filtering 80%/20% of raw angular velocities without offsets
    gyro_filtered[ROLL]=gyro_filtered[ROLL]*0.8+gyro_raw[ROLL]*0.2;
    gyro_filtered[PITCH]=gyro_filtered[PITCH]*0.8+gyro_raw[PITCH]*0.2;
    gyro_filtered[YAW]=gyro_filtered[YAW]*0.8+gyro_raw[YAW]*0.2;
        
    // calculation of the raw angular velocities without offsets and
    // filtered in [°/s].
...
void calculateSetpoints()
{
    ...
    setpoint[PITCH] = 0;
	  ...
	  // the Pitch setpoint calculation curve must have a negative slope so that
	  // the UAV's reaction corresponds to the requested movements.
...
void waitMinGas()
{
		// the "while" loop runs and creates a wait as long as the receiver does not receive anything
		// or the joystick is not in the expected safe position
		//
		// this precaution is necessary so that the motors do not start rotating as soon
		// as the quadcopter is switched on.
		//
		// as a safety position, the left joystick (in radio control mode 2) is expected to be at the
		// bottom and in the middle, i.e. : 
		//
		//		- GAS control at minimum
		//		- YAW control in median position
		//
		// Note: if the receiver does not receive anything, all the variables containing the PWM pulse
		// durations are zero because they were initialized to zero at the beginning of the program.
		//
    // in the safe position, the expected values of the PWM pulse durations are :
		//
		//			- 1000us for GAS
		//			- 1500us for YAW
    //
		// taking into account the imperfections of the signals, we consider that the GAS are at minimum if
		// the Channel 3 pulses are between 990us and 1020us i.e. : 990 < pulse_duration [CHANNEL3] < 1020
		//
		// in the same way we consider that the YAW is at minimum if the Channel 4 pulses are between
		// 1450us and 1550us that is : 1450 < pulse_duration [CHANNEL4] < 1550
		//
		// the left joystick is not in the safety position if :
		//
		//	- 	pulse_duration[CHANNEL3] < 990
		//	-	  or pulse_duration[CHANNEL3] > 1020
		//	- 	or pulse_duration[CHANNEL4] < 1450
		//	- 	or pulse_duration[CHANNEL4] > 1550 
		//
		// for these pulse durations, which correspond to any other position than the safety position,
		// we stay in the "while" loop : this is what creates the waiting

    // Reminder of channel assignments :
    //
    //      ROLL   --> Channel 1  
    //      PITCH  --> Channel 2
    //      GAS    --> Channel 3
    ...
    while(pulse_duration[CHANNEL3] <990 || pulse_duration[CHANNEL3] > 1020 || pulse_duration[CHANNEL4] < 1450 || pulse_duration[CHANNEL4] > 1550 )      
    {
  			cpt ++;	// the variable "cpt" is incremented at each pass in the loop
  		  
  			// if the ESCs are powered on but do not receive a PWM control signal, they emit "beeps".
        // we don't want the ESCs to beep continuously waiting for the correct positioning of the joystick,
        // so we send them a minimum control pulse (1000us) while waiting for the right values from the radio receiver
